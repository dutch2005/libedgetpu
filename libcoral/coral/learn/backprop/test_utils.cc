...
TrainingData ShuffleAndSplitData(const MatrixXf& data_matrix,
  ...
  shuffled_data = data_matrix(shuffled_indices, Eigen::placeholders::all);
  ...
  fake_data.training_data =
      shuffled_data(Eigen::seq(0, num_train - 1), Eigen::placeholders::all);
  fake_data.validation_data =
      shuffled_data(Eigen::seq(num_train, Eigen::last), Eigen::placeholders::all);
...
TrainingData GenerateMvnRandomData(const std::vector<int>& class_sizes,
  ...
  for (int i = 0; i < class_sizes.size(); i++) {
    ...
    // Eigen::seq boundaries are inclusive on both sides.
    data_matrix(Eigen::seq(start_index, start_index + n - 1), Eigen::placeholders::all) =
        samples.transpose();
